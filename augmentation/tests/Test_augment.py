...
for samples in range(1000):
  for ch in range(3):
      if samples < 500:
          data[samples][ch]*= samples 
      if samples >= 500:
          data[samples][ch]*= (1000 - samples) 
...
stretching_gap = 3
dim = data.shape[0]
spt = np.random.randint(50,300)
sst = np.random.randint(300,700)
coda_end = np.random.randint(700,1000)

stretch_data =  augment._stretch(data, stretching_gap)
spt2, sst2, codaend2 = augment._stretchLabel(spt, sst, coda_end, stretching_gap)
slide_data =  augment._slideCut(data, dim)
spt3, sst3, codaend3 = augment._slideCutLabel(spt, sst, coda_end, dim)
warp_data = augment._magnitudeWarp(data)
...
def testStretchData():
    ...
    if (len(data) * (stretching_gap - 1)) % stretching_gap == 0:
        assert len(stretch_data) == len(data) * stretching_gap, "Length error: Stretching"
    else:
        rest = (len(data) * (stretching_gap - 1)) % stretching_gap
        control = len(data) * stretching_gap - rest
        assert len(stretch_data) == control, "Length error: Stretching"

    assert len(stretch_data[0]) == len(data[0]), "Missing channel error"


def testSlideData():
    slide_data = augment._slideCut(stretch_data, dim)

    assert dim == len(slide_data), "Length error: Sliding and Stretching"
    assert len(stretch_data[0]) == len(slide_data[0]), "Missing channel error"


def testStretchlabel():
    spt2, sst2, codaend2 = augment._stretchLabel(spt, sst, coda_end, stretching_gap)

    if spt and sst:
        assert sst2 - spt2 == (sst - spt) * stretching_gap, "P or S was labeled wrong!"
    if spt and coda_end:
        assert codaend2 - spt2 == (coda_end - spt) * stretching_gap, "P or coda end was labeled wrong!"


def testSlidelabel():
    spt3, sst3, codaend3 = augment._slideCutLabel(spt2, sst2, codaend2, dim)

    if spt2 and sst2:
        assert sst3 - spt3 == sst2 - spt2, "P or S was labeled wrong!"
    if codaend2 - spt2 - 500 <= len(slide_data):
        assert codaend3 - spt3 == codaend2 - spt2, "P or coda end was labeled wrong!"


def testMagnitudeWarp():
    warp_data = augment._magnitudeWarp(data)

    assert len(warp_data[0]) == len(data[0]), "Length error"
    assert len(warp_data[1]) == len(data[1]), "Missing channel error"
